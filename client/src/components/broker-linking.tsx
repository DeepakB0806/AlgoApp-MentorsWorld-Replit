                      ...
                      {!deployConfig[plan.id] ? (
                        ...
                        <div className="space-y-3" data-testid={`container-deploy-config-${plan.id}`}>
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            ...
                            {deployConfig[plan.id].tslEnabled && deployConfig[plan.id].tslType === "amount" && (
                              <>
                                <div>
                                  <Label className="text-xs mb-1 block text-muted-foreground">
                                    TSL Activate At
                                    {deployConfig[plan.id].baseTslActivateAt > 0 && (
                                      <span className="ml-1 text-muted-foreground/60">(base: {deployConfig[plan.id].baseTslActivateAt})</span>
                                    )}
                                  </Label>
                                  <Input
                                    type="number"
                                    value={deployConfig[plan.id].tslActivateAt}
                                    onChange={(e) => setDeployConfig((prev) => ({
                                      ...prev,
                                      [plan.id]: { ...prev[plan.id], tslActivateAt: parseFloat(e.target.value) || 0 },
                                    }))}
                                    data-testid={`input-deploy-tsl-activate-${plan.id}`}
                                  />
                                </div>
                                <div>
                                  <Label className="text-xs mb-1 block text-muted-foreground">
                                    TSL Lock Profit At
                                    {deployConfig[plan.id].baseTslLockProfitAt > 0 && (
                                      <span className="ml-1 text-muted-foreground/60">(base: {deployConfig[plan.id].baseTslLockProfitAt})</span>
                                    )}
                                  </Label>
                                  <Input
                                    type="number"
                                    value={deployConfig[plan.id].tslLockProfitAt}
                                    onChange={(e) => setDeployConfig((prev) => ({
                                      ...prev,
                                      [plan.id]: { ...prev[plan.id], tslLockProfitAt: parseFloat(e.target.value) || 0 },
                                    }))}
                                    data-testid={`input-deploy-tsl-lock-${plan.id}`}
                                  />
                                </div>
                                <div>
                                  <Label className="text-xs mb-1 block text-muted-foreground">
                                    When Profit Increases By
                                    {deployConfig[plan.id].baseTslWhenProfitIncreaseBy > 0 && (
                                      <span className="ml-1 text-muted-foreground/60">(base: {deployConfig[plan.id].baseTslWhenProfitIncreaseBy})</span>
                                    )}
                                  </Label>
                                  <Input
                                    type="number"
                                    value={deployConfig[plan.id].tslWhenProfitIncreaseBy}
                                    onChange={(e) => setDeployConfig((prev) => ({
                                      ...prev,
                                      [plan.id]: { ...prev[plan.id], tslWhenProfitIncreaseBy: parseFloat(e.target.value) || 0 },
                                    }))}
                                    data-testid={`input-deploy-tsl-step-${plan.id}`}
                                  />
                                </div>
                                <div>
                                  <Label className="text-xs mb-1 block text-muted-foreground">
                                    Increase TSL By
                                    {deployConfig[plan.id].baseTslIncreaseTslBy > 0 && (
                                      <span className="ml-1 text-muted-foreground/60">(base: {deployConfig[plan.id].baseTslIncreaseTslBy})</span>
                                    )}
                                  </Label>
                                  <Input
                                    type="number"
                                    value={deployConfig[plan.id].tslIncreaseTslBy}
                                    onChange={(e) => setDeployConfig((prev) => ({
                                      ...prev,
                                      [plan.id]: { ...prev[plan.id], tslIncreaseTslBy: parseFloat(e.target.value) || 0 },
                                    }))}
                                    data-testid={`input-deploy-tsl-inc-${plan.id}`}
                                  />
                                </div>
                              </>
                            )}
