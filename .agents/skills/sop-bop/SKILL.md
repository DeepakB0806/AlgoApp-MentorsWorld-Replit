...
- Add broker API client module to `artifacts/api-server/src/` (e.g., `artifacts/api-server/src/kotak-neo-api.ts`)
...
- Add fields to the `ApiFieldsReference` component in `artifacts/mentors-world/src/pages/broker-api.tsx`
...
- Create `artifacts/api-server/src/tl-{broker-name}.ts` (e.g., `artifacts/api-server/src/tl-kotak-neo-v3.ts`)
...
## Naming Conventions
- SDK file: `artifacts/api-server/src/{broker-name}-api.ts`
- Translation Layer: `TL_{BrokerName}` (e.g., `TL_KotakNeoV3`)
- TL file: `artifacts/api-server/src/tl-{broker-name}.ts`
