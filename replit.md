...
- **Trade Engine (TE) Fully DB-Driven**: The TE (`te-kotak-neo-v3.ts`) is fully database-driven. It passes only 5 dynamic universal field values per order (`tradingSymbol`, `exchange`, `transactionType`, `quantity`, `productType`). All static defaults (`price=0`, `priceType=MKT`, `validity=DAY`, `afterMarketOrder=NO`, `disclosedQuantity=0`, `marketProtection=0`, `priceFillFlag=N`, `triggerPrice=0`) are stored in `broker_field_mappings.default_value` and auto-filled by the TL via `buildRequestPayload(category, payload, includeDefaults=true)`. Transaction type mapping (`BUY→B`, `SELL→S`) is stored in `broker_field_mappings.allowed_values` and read via `TL.mapValueFromAllowed()`. Zero hardcoded broker field codes, zero hardcoded default values in the TE.
